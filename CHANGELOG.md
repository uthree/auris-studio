...
* **The Namine Ritsu preparation writes its labels too.** The database's hand-corrected mono
  labels were read for the phrase boundaries alone; `prepare_namine_ritsu.py` now writes them
  out as seconds per phoneme beside each phrase, and the preprocessing config names the
  folder, so the voice trains on the labels the way JSUT-song does.

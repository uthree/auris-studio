...
pub mod harmony {
    ...
    //! Source tab edits the stored text by itself. Their read-only Overlay tab adds the performed
    //! notes faintly for direct comparison, while the read-only Performance tab shows that result
    //! by itself, including each loop pass. Both performed views refresh during parameter gestures.

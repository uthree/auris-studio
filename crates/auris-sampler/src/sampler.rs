...
mod tests {
    ...
    #[test]
    fn all_sound_off_controller_clears_note_ownership_before_the_next_render() {
        let mut instrument = playing(stocked(), 0, 4096);
        let mut out = AudioBuffer::stereo(4096, RATE);
        let ctx = ProcessContext::realtime(RATE, 4096, 0, 120.0, true);
        // The killed voice remains in the pool until rendering. Its old ownership must not
        // consume the note-off belonging to the new note at the same channel and pitch.
        instrument.process(
            &[
                note_on(0),
                NoteEvent::Controller {
        ...
        assert!(
            rms(&out.channel(0)[2048..]) < 1e-6,
            "the note started after All Sound Off kept sounding after its own note-off"
        );
    }

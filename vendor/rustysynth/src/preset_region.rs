...
/// Adds a zone's modulators to what is already there, replacing rather than repeating.
///
/// Added by the Auris fork. A local zone's modulator overrides the global zone's when the two
/// address the same source and destination — see [`Modulator::addresses_the_same`].
pub(crate) fn merge_modulators(into: &mut Vec<Modulator>, zone: &[Modulator]) {
    for modulator in zone {
        match into.iter_mut().find(|m| m.addresses_the_same(modulator)) {
            Some(existing) => *existing = *modulator,
            None => into.push(*modulator),
        }
    }
}
...
pub struct PresetRegion {
    ...
    /// Added by the Auris fork: what this zone's controllers do to those generators.
...
impl PresetRegion {
    fn new(
        ...
        merge_modulators(&mut modulators, &global.modulators);
        merge_modulators(&mut modulators, &local.modulators);

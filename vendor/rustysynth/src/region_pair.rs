...
impl<'a> RegionPair<'a> {
    ...
    /// A generator with this region's modulators applied, for a note of `key` at `velocity`.
    ///
    /// Added by the Auris fork. A modulator is an *offset* on the generator it names, and a
    /// preset's are additive to its instrument's exactly as their generators are — so this is the
    /// same sum as [`Self::gs`] with the controllers added on top.
    ///
    /// Only the two filter destinations are read through this. What a font says about *loudness*
    /// with a modulator is left alone, because [`Voice::start`](crate::voice::Voice::start) already
    /// applies the specification's default velocity-to-attenuation curve by hand: applying both
    /// would count a font's own velocity shaping twice.
    pub(crate) fn modulated(&self, i: usize, key: i32, velocity: i32) -> f32 {
        let from = |modulators: &[Modulator]| -> f32 {
            modulators
                .iter()
                .filter(|modulator| modulator.destination as usize == i)

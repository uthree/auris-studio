...
impl SoundFontParameters {
    pub(crate) fn new<R: Read>(reader: &mut R) -> Result<Self, SoundFontError> {
        ...
        // Added by the Auris fork. A font with no modulator chunk at all is legal and reads as an
        // empty list rather than as a missing one, which is why these have no `ok_or` below.
        ...
        let instrument_zones = Zone::create(
            ...
            &instrument_modulators.unwrap_or_default(),
        )?;
        let instruments =
        ...
        let preset_zones = Zone::create(
            &preset_bag,
            &preset_generators,
            &preset_modulators.unwrap_or_default(),
        )?;

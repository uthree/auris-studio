...
impl Voice {
    ...
    pub(crate) fn start(&mut self, region: &RegionPair, channel: i32, key: i32, velocity: i32) {
        ...
        // The two filter values read through the font's modulators rather than off its generators
        // — the Auris fork's whole reason for existing. A font that sets a low cutoff and opens it
        // with a controller is the ordinary way to make a sound respond to how hard it is played;
        // reading the generator alone leaves the filter shut. See `modulator.rs`.
        self.cutoff = SoundFontMath::cents_to_hertz(region.modulated(
            GeneratorType::INITIAL_FILTER_CUTOFF_FREQUENCY as usize,
            key,
            velocity,
        ));
        ...
        self.mod_env_to_cutoff = region.modulated(
            GeneratorType::MODULATION_ENVELOPE_TO_FILTER_CUTOFF_FREQUENCY as usize,
            key,
            velocity,
        ) as i32;

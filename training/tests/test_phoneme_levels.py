...
def test_a_consonant_is_measured_against_the_vowel_after_it():
    ...
    assert "a" not in levels and "<sil>" not in levels, (
        "vowels are the reference, specials boundaries"
    )


def test_a_consonant_with_no_vowel_after_it_or_at_the_floor_is_not_a_reading():
    levels = measure(
        [utterance([("a", 10, 0.1), ("ɴ", 5, 0.05)]), utterance([("k", 4, 0.0), ("a", 10, 0.1)])]
    )
    assert levels == {}


def test_a_devoiced_vowel_is_measured_and_skipped_as_a_reference():
    levels = measure([utterance([("s", 4, 0.02), ("ɯ̥", 5, 0.01), ("a", 20, 0.1)])])
...
def test_the_block_ships_the_medians_that_earned_it_and_a_consonant_default():
    ...
    assert table["default"] == pytest.approx(-21.0), (
        "the pooled median, a consonant's level, not 0 dB"
    )
...
def test_the_export_carries_the_table_and_refuses_a_stranger(tiny_model_config, tmp_path):
    ...
    with pytest.raises(ValueError, match="phoneme_levels for x names symbols"):
        export_onnx(
            AurisSinger(**tiny_model_config).eval(),
            tmp_path / "w.onnx",
            metadata=metadata,
            phoneme_levels={
                **table,
                "speakers": {"x": {"default": -12.0, "db": {"not-a-model-symbol": -22.0}}},
            },
        )
    with pytest.raises(ValueError, match="speakers the model has not"):
        export_onnx(
            AurisSinger(**tiny_model_config).eval(),
            tmp_path / "w.onnx",
            metadata={**metadata, "speaker_to_id": {"y": 0, "z": 1}},
            phoneme_levels=table,
        )
    with pytest.raises(ValueError, match="format-1 table"):
        export_onnx(
            ...
            tmp_path / "w.onnx",
            metadata=metadata,
            phoneme_levels={"unit": "db", "default": -12.0, "db": {"k": -22.0}},
        )
